...
function decorateReels(root=document){root.querySelectorAll('.reel-card[data-drive-video]').forEach(card=>{const src=reelSource(card);if(src)card.dataset.videoSrc=src;const poster=card.querySelector('.reel-poster');if(!poster||poster.querySelector('.reel-thumb-img'))return;const img=document.createElement('img');img.className='reel-thumb-img';img.loading='lazy';img.alt='';img.src=`https://drive.google.com/thumbnail?id=${card.dataset.driveVideo}&sz=w800`;img.addEventListener('error',()=>img.remove(),{once:true});poster.prepend(img)})}
document.addEventListener('click',e=>{const y=e.target.closest('[data-youtube]');const r=e.target.closest('[data-drive-video]');if(!y&&!r)return;e.preventDefault();e.stopImmediatePropagation();if(y){const title=y.closest('article')?.querySelector('h3')?.textContent||y.querySelector('strong')?.textContent||'Video';openYoutube(y.dataset.youtube,title)}else openReel(r.dataset.driveVideo,r.dataset.title||r.querySelector('strong')?.textContent||'Reel',reelSource(r))},true);
